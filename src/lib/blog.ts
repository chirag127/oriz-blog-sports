...
export function verticalFor(category: string): Vertical {
  ...
  if (/(fitness|training|strength|conditioning|nutrition|recovery|athlet)/.test(c)) return 'finance'
  if (/(tactic|strategy|formation|coaching|playbook|set-piece|drill)/.test(c)) return 'dev'
  if (/(analysis|stat|scorecard|data|metric|numbers|breakdown|review)/.test(c)) return 'culture'
  if (/(culture|history|fans|rivalr|off-field|lifestyle|profile|story)/.test(c)) return 'health'
  return 'personal'
}
